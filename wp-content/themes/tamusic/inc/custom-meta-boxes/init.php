		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'ta-meghna' ),
					'defaultString' => __( 'Default', 'ta-meghna' ),
					'pick'          => __( 'Select Color', 'ta-meghna' ),
					'current'       => __( 'Current Color', 'ta-meghna' ),
				) );
			}
		} else {
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'ta-meghna' ) );
